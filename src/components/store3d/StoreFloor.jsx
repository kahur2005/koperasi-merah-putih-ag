    ...
    <group ref={groupRef} position={position} rotation={rotation} onPointerMove={onPointerMove} onClick={onClick} onDoubleClick={onDoubleClick}>
      {hasDoor ? (
        <>
          <mesh position={[0, height * 0.75, 0]} receiveShadow>
            <boxGeometry args={[length, height * 0.5, 0.5]} />
            <meshStandardMaterial color="#c92f2d" roughness={0.82} />
          </mesh>
          <mesh position={[-(length/4 + 1), height * 0.25, 0]} receiveShadow>
            <boxGeometry args={[length/2 - 2, height * 0.5, 0.5]} />
      ...
      {hasText && (
        <Text
          ...
          outlineWidth={0.02}
          outlineColor="#991B1B"
          font="/assets/fonts/EightBitDragon-anqx.ttf"
        >
