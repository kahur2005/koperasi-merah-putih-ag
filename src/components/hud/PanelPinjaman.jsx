    ...
    return (
      <div className="modal-overlay" onClick={handleClose}>
        <div className="modal-content glass-card" onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ display: 'flex', gap: '26px', alignItems: 'center', margin: '12px 0' }}>
            <img 
              src={avatar} 
              alt={loanName} 
            ...
            />
            <div>
              <h3 style={{ fontSize: '23px', marginBottom: '2px' }}>{loanName}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '17px' }}>{selectedLoan.pekerjaanAnggota || selectedLoan.pekerjaan || 'Anggota Koperasi'}</p>
              <p style={{ color: 'var(--text-secondary)', fontSize: '16px' }}>{UI.PENDAPATAN_BULANAN}: <span style={{ color: 'var(--accent-green)', fontWeight: '600' }}>{formatRupiah(selectedLoan.pendapatanBulanan)}</span></p>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '13px', margin: '12px 0', fontSize: '17px' }}>
          ...
          <div className="info-note" style={{ fontSize: '14px', borderLeft: '8px solid var(--accent-orange)' }}>
          ...
          <div className="glass-card" style={{ marginBottom: '16px', padding: '16px' }}>
            <h4 style={{ fontSize: '16px', textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: '8px' }}>{UI.PERHITUNGAN_CICILAN}</h4>
      ...
      <div className="modal-overlay" onClick={handleClose}>
        <div className="modal-content glass-card" style={{ maxWidth: '700px' }} onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '26px' }}>
            {/* List of members */}
            <div>
              <h3 style={{ fontSize: '20px', color: 'var(--text-secondary)', textTransform: 'uppercase', marginBottom: '8px' }}>Daftar Anggota Koperasi ({members.length})</h3>
              {members.length === 0 ? (
                <div className="info-note" style={{ textAlign: 'center', fontSize: '17px', color: 'var(--text-secondary)' }}>
                  Koperasi belum memiliki anggota. Terima anggota baru di Bottom Bar!
                </div>
              ) : (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', maxHeight: '180px', overflowY: 'auto' }}>
                  {members.map((m) => (
                    <div key={m.id} className="glass-card" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '6px 12px', flex: '1 0 200px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ fontSize: '16px', fontWeight: '600' }}>{getPersonName(m)}</span>
                        <span style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{m.pekerjaan}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Active loans */}
            <div>
              <h3 style={{ fontSize: '20px', color: 'var(--text-secondary)', textTransform: 'uppercase', marginBottom: '8px' }}>Pinjaman Aktif ({activeLoans.filter(l => l.status === 'aktif').length})</h3>
              {activeLoans.filter(l => l.status === 'aktif').length === 0 ? (
                <div className="info-note" style={{ textAlign: 'center', fontSize: '17px', color: 'var(--text-secondary)' }}>
                  Tidak ada pinjaman aktif saat ini.
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '13px', maxHeight: '220px', overflowY: 'auto' }}>
                  {activeLoans.filter(l => l.status === 'aktif').map((l) => {
                    const progressPercent = Math.round(((l.tenorBulan - l.sisaBulan) / l.tenorBulan) * 100);
                    return (
                      <div key={l.id} className="glass-card" style={{ padding: '16px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                          <span style={{ fontSize: '17px', fontWeight: '700' }}>{getPersonName(l)}</span>
                          <span style={{ fontSize: '14px', color: 'var(--accent-orange)' }}>Sisa {l.sisaBulan} dari {l.tenorBulan} bulan</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '16px', color: 'var(--text-secondary)', marginBottom: '8px' }}>
